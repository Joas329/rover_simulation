...
class VelPID(Node):
    ...
    def __init__(self):
        ...
        self.desiredWheelPositionIndexingTImer = self.create_timer(0.1,self.desiredWheelPositionIndexing) #TODO YOU CAN CHANGE THE TIMER TIME HERE
        ...
        self.joint_trajectory_subscriber = self.create_subscription(
            Float64MultiArray,
            'wheelDesiredPositions',
            self.wheel_desired_position_callback,
            10
        )
    ...
    def wheel_desired_position_callback(self, msg):
    ...
    def desiredWheelPositionIndexing(self):
        #This indexes through trajectory array
        if  not self.stack[0] == None:
            if self.trajectoryFlag == False:
                print("Receiving Desired Wheel Positions")
                self.trajectoryFlag = True
            if not len(self.stack) == 0: 
                self.desired_joint_positions = self.stack.pop()
            else:
                print("Not Receiving any more desired positions")
    ...
    def pidVelocityCalc(self, jointDesiredPositions, joints_number):
        ...
        #Uncomment once done tunning PID from position --> velocity
